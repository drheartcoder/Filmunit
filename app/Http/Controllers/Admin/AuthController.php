...
class AuthController extends Controller
{
  ...
  public function process_login(Request $request)
  {
     $validator = Validator::make($request->all(), [
      ...
      if($check_authentication)
      {
        ...
        elseif($user->inRole(config('app.project.role_slug.subadmin_role_slug')))
        {
          return redirect(config('app.project.admin_panel_slug').'/dashboard');
        }
        else
  ...
  public function built_mail_data($email, $reminder_code)
  {
    ...
    if($user)
    {
        ...
        $reminder_url = URL::to($this->admin_panel_slug.'/validate_admin_reset_password_link/'.base64_encode($arr_user['id']).'/'.base64_encode($reminder_code) );
