...
class PhotosAndFootageController extends Controller
{
    ...
    public function index()
    {
        ...
        $user = Sentinel::check();
       	...
       	{
       		$seller_id    = $user['id'];
       		$obj_arr_data = $this->MediaListingMasterModel->where('seller_id',$seller_id)->orderBy('id','DESC')->where('is_admin_uploaded',0)->paginate(10);
    ...
    public function index_admin()
    {
        ...
        $user = Sentinel::check();
       	...
       	{
       		$seller_id    = $user['id'];
       		$obj_arr_data = $this->MediaListingMasterModel->where('seller_id',$seller_id)->where('is_admin_uploaded',1)->orderBy('id','DESC')->paginate(10);
       		if($obj_arr_data!=false)
       		{
                $arr_pagination   = $obj_arr_data;
       			$arr_data         = $obj_arr_data->toArray();
