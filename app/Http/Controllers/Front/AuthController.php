...
/*------------------------ Init function for login process ----------------------------*/
	public function init_login(array $arr_credentials, $check_activation = true, $remember_me = null, $social_login = false, $redirect_from = false, $back_url)
	{
		...
		if(isset($arr_credentials['email']) && isset($arr_credentials['password']) /*&& isset($arr_credentials['user_type'])*/)
		{
		  ...
		  if($obj_user->is_active == 0 && $obj_user!=null)
		  {
		    ...
		    if(count($arr_roles) > 0) 
        	{
	      	  if(isset($arr_roles[0]) && ($arr_roles[0] != ""))
			  {
	    		...
	    		if($back_url!='')
	    		{
	    			...
	    			if((isset($back_url_segment[5]) && $back_url_segment[5]!='') )
	    			{
	    				if(($back_url_segment[5]=='listing' || $back_url_segment[5]=='cart' || $back_url_segment[5]=='listing?type=photo' || $back_url_segment[5]=='listing?type=footage' ) && $arr_roles[0]=='buyer')
	    				{
	    		...
	    		//Check whether terms and conditions are accepted or Not

	    		if($obj_user!=false  && $this->UserModel->where('id',$obj_user['id'])->where('terms',0)->count()==1)
	    		{	
	    			$this->arr_view_data['check_conditions']    = 1;
					$this->arr_view_data['title']    			= "Terms and Conditions";				
					return view('front.terms_conditions',$this->arr_view_data);		
	    		}		

	    		//If terms and conditions are accepted then redirect it to respective dashboard
    ...
    /*------------- To check whether terms and conditions are accepted or not --------------*/

	public function check_terms_conditions(Request $request)
	{
		$status = '';
		$user   = false;
		$role   = '';
		...
		if($status=='accept' && $user!=false)
		{
			$role 	 = $user['role'];
			$user_id = $user['id'];

			$update_terms = $this->UserModel->where('id',$user_id)->update(['terms'=>1]);
			if($role=='buyer' && $update_terms)
    		{
    			$this->add_cart_item_to_user($user_id,$visitor_id);
	        	return redirect(url('buyer/dashboard'));
    		}
    		else if($role=='seller' && $update_terms)
    		{
    			$delete_cart = $this->CartModel->where('visitor_id',$visitor_id)->delete();
	            return redirect(url('seller/dashboard'));
    		}
