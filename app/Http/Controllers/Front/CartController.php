...
class CartController extends Controller
{
	...
	public function index()
	{
        ...
        {
          $buyer_id = $user['id'];
          $obj_data = $this->CartModel->where('buyer_id',$buyer_id)->with('listing_details.format_details')->with('master_details.listing_details.format_details')->orderBy('updated_at','DESC')->get();
        }
        else
        {
          $obj_data = $this->CartModel->where('visitor_id',$visitor_id)->with('listing_details.format_details')->with('master_details.listing_details.format_details')->orderBy('updated_at','DESC')->get();
        }
    ...
    /*------------- Fetch values for add to Cart Popup --------------*/

    public function get_sublisting(Request $request) 
    {
        $master_id           = '';
        $obj_master_details  = false;
        $arr_master_details  = [];

        if($request->input('enc_master_id')!='')
        {
            $master_id = base64_decode($request->input('enc_master_id'));  

            $obj_master_details = $this->MediaListingMasterModel->where('id',$master_id)->with('listing_details.format_details')->first();
            
            if($obj_master_details!=false)
            {
                $arr_master_details  = $obj_master_details->toArray();  
            }
        }
        return response(array("arr_master_details" => $arr_master_details));
    }              	    	
}
