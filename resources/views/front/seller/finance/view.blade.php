            ...
            <div class="row">
                ...
                @if(isset($arr_data) && count($arr_data)>0)
                    <div class="light-grey-bg-block">
                        <h1 class="accou-heat"> Finance </h1>
                           ...
                           <table class="table border-none">
                               ...
                               <tr>
                                   ...
                                   <td class="table-cell-break">{{isset($value['order_number'])?substr($value['order_number'],0,10).'...':'NA'}}</td>
                                   ...
                                   <td >{{isset($value['listing_details']['master_details']['commission'])?$value['listing_details']['master_details']['commission'].' %':'NA'}}</td>
                                   <td >{{isset($value['listing_details']['master_details']['commission'])?' $'.sprintf("%.2f", $value['price']-($value['price']*$value['listing_details']['master_details']['commission']/100)):'NA'}}</td>                </tr>
                               @endforeach
                             </table>
                           </div>
                      <div class="clr"></div> 
                     ...
                     <div class="button-section right-side tp-btns"><a href="{{ url('/')}}/seller/finance">Back</a> </div>
              <!-- Paination Links -->
              @else
              <h3 style="text-align: center; color: #FFF">No records Found.</h3>
              @endif
                </div>
            </div>
        </div>
