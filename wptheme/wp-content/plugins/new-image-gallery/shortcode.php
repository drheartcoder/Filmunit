...
function awl_image_gallery_shortcode($post_id) {
	...
	wp_enqueue_script('awl-ig-bootstrap-js', IG_PLUGIN_URL .'js/bootstrap.min.js', array('jquery'), '' , true);
	wp_enqueue_script('awl-imagesloaded-pkgd-js', IG_PLUGIN_URL .'js/imagesloaded.pkgd.js', array('jquery'), '' , true);
	wp_enqueue_script('awl-ig-isotope-js', IG_PLUGIN_URL .'js/isotope.pkgd.min.js', array('jquery'), '', false);
	
	// bootstrap css
	//wp_enqueue_style('awl-bootstrap-css', IG_PLUGIN_URL .'css/bootstrap.css');
	
	// awp custom bootstrap css
	wp_enqueue_style('awl-bootstrap-css', IG_PLUGIN_URL .'css/ig-bootstrap.css');
	...
	if($image_hover_effect_type == "no") {
		...
		// hover csss
		wp_enqueue_style('ggp-hover-css', IG_PLUGIN_URL .'css/hover.css');
	}
	...
	#image_gallery_<?php echo $image_gallery_id; ?> .thumbnail {
		...
		border-radius: 0px;
		/*background: transparent url('<?php echo IG_PLUGIN_URL.'img/loading.gif'; ?>') center no-repeat !important;*/
	}
	<?php if($no_spacing) { ?>
