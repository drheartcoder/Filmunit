...
require_once(THEMESFLAT_DIR .'inc/options/controls/switcher.php');
...
require_once(THEMESFLAT_DIR .'inc/options/controls/background.php');
require_once(THEMESFLAT_DIR .'inc/options/controls/widgets-layout.php');
...
//echo'hello'; die;
function get_our_package() {
    global $wpdb;
    $totalRows = [];
    $totalRows = $wpdb->get_results( "SELECT id, title,slug,enc_image_name,is_active FROM packs where is_active='1'");

    $myrows = $wpdb->get_results( "SELECT id, title,slug,enc_image_name,is_active,order_number FROM packs where is_active='1' AND order_number > 0 ORDER BY order_number limit 6 " );
?>
	...
	<div class="categories-main-block">
	       ...
	       <div class="row grid">
	    	...
	    	{
			    foreach ($myrows as $key => $master_list)
			    {
			    	$image_name = '';
			    	$list_id = $master_list->id;
		    		$image_name = isset($master_list->enc_image_name)?$master_list->enc_image_name:'';          		
			?>                
	          		<div class="col-xs-6 col-sm-4 col-md-4 col-lg-4">
                         <div class="common-effect effect-zoe margin-30px">
						<a href="<?php echo BASE_URL.'package/'.$master_list->slug ?>"><img src="<?php echo DB_IMG.$image_name; ?>" alt="<?php echo ucfirst($master_list->title); ?>"/>
						<div class="content-hover">
							<h2><?php echo ucfirst($master_list->title); ?></h2>
							
						</div>	</a>		
					</div>
                        
		          		<!--<a href="<?php echo BASE_URL.'package/'.$master_list->slug ?>">
		                 	<div class="main-block-cate ">
		                        <div class="text-img"><?php echo ucfirst($master_list->title); ?>
		                        </div>
		                        <span class="effect-zoe">
		                         	<img src="<?php echo DB_IMG.$image_name; ?>" alt="<?php echo ucfirst($master_list->title); ?>">
		                            <span class="content-hover">	
			                            <span class="view-txt-more">
				                            <span class="hover-text"><?php echo ucfirst($master_list->title); ?>
				                            </span>
				                        </span>
			                    	</span>
		                        </span>
	       ...
	       <div class="clr"></div>
	       <?php if(isset($totalRows) && count($totalRows)>6) { ?> 
       		<a class="new-btn" href="<?php echo BASE_URL.'package'; ?>">View All</a>
   		 	<?php } ?>
	</div>
<?php return;
}
...
function get_checking(){
   add_shortcode('category-collections', 'get_our_package');
}
add_action( 'init', 'get_checking');
...
function new_images() {
	...
	global $wpdb;
    //print_r($wpdb); die;
    $myrows = $wpdb->get_results( "SELECT id, type, title, admin_enc_item_name,admin_enc_footage_image,slug FROM media_listing_master where type='footage' AND is_approved='1' ORDER BY RAND() limit 12 " );
  // /  print_r($myimages); die;
  //  echo url('/subscriber'); die; 
	if(isset($myrows) && count($myrows)>0)
	{
?>
	<div class="reproduced-section">
      <div class="container">
          	<ul id="flexiselDemo1" class="grid">
<?php
		    foreach ($myrows as $key => $master_list)
		    {
		    	$footage_name 	  = '';
		    	...
		    	$list_id 		  = $master_list->id;
	    		$list_details 	  = $wpdb->get_results( "SELECT id, list_id,price,enc_item_name FROM media_listing_details where list_id=$list_id ORDER BY RAND() limit 1 " );
	    		
	    		$footage_name 	  = isset($master_list->admin_enc_footage_image)?$master_list->admin_enc_footage_image:'';           		
	    		$max_price 		  = $wpdb->get_results( "SELECT MAX(price) AS price FROM media_listing_details where list_id=$list_id " );
	    		$min_price 		  = $wpdb->get_results( "SELECT MIN(price) AS price FROM media_listing_details where list_id=$list_id " );
?>
				<a href="<?php echo BASE_URL.'listing/view/'.$master_list->slug ?>">
                <li class="listing-box">
                 <div class="common-effect effect-zoe">
						<img src="<?php echo DB_Footage.$footage_name; ?>" alt="<?php echo ucfirst($master_list->title); ?>"  alt="<?php echo ucfirst($master_list->title); ?>"/>
						<div class="content-hover">
							<div class="icon-links">
                              
                                <a href="<?php echo BASE_URL.'listing/view/'.$master_list->slug ?>"> <span class="name-latest"><?php echo ucfirst($master_list->title); ?></span> <i class="fa fa-eye" title="view"></i></a>
                 ...
                 <!--<div class="preview-panel"> 
                       <img src="<?php echo DB_Footage.$footage_name; ?>" alt="<?php echo ucfirst($master_list->title); ?>"  alt="<?php echo ucfirst($master_list->title); ?>"/>
                        <div class="preview-panel-desc"><div class="preview-panel-left"> <h4 class="gold-color"><?php echo ucfirst($master_list->title); ?></h4> </div>
                                 
                                  <div  class="preview-panel-right">
                                             <p class="format-price"> <?php
                             if(isset($min_price[0]) && isset($max_price[0]))
                             { 
	                             if($min_price[0]->price == $max_price[0]->price)
	                             {
	                             ?>
	                             $<?php echo $max_price[0]->price; ?>
	                             <?php } else { ?>                             
	                             $<?php echo $min_price[0]->price; ?> - $<?php echo $max_price[0]->price; ?>
	                             <?php
	                         	 }
	                         }
                             ?></p>
                  ...
                  <!--<div class="main-block-cate ">
                    ...
                    <span class="effect-zoe">
	                    ...
	                    <img src="<?php echo DB_Footage.$footage_name; ?>" alt="<?php echo ucfirst($master_list->title); ?>">

                    <span class="over-txt-block">	
                    <span class="view-txt-more">
					<a href="<?php echo BASE_URL.'listing/view/'.$master_list->slug ?>" ><i class="fa fa-eye" title="view"></i></a>
					</span>
                    <span class="footer-hover">
                       <span class="flex-hover-cont">
                            <span class="pull-leftss">
                                <span class="title-small"><span class="latest-c"><?php echo ucfirst($master_list->title); ?></span></span>
                            </span>
                            <span class="pull-rightss">
                             ...
                             { 
	                             ...
	                             {
	                             ?>
	                             <span>$<?php echo $max_price[0]->price; ?></span>
	                             <?php } else { ?>                             
	                             <span>$<?php echo $min_price[0]->price; ?> - $<?php echo $max_price[0]->price; ?></span></span>
	                             <?php
	                         	 }
	                         }
                             ?>
...
//add_action( 'wp_enqueue_scripts', 'new_images' );

function get_check_gallery(){
   add_shortcode('latest-category', 'new_images');
}
add_action( 'init', 'get_check_gallery');
...
<!--    Subscribe Section Start Here-->
    ...
    <form method="POST" action="<?php echo BASE_URL.'subscriber'; ?>" >
         <div class="subscribe-section" style="padding: 30px 0 5px; margin: 0 -15px;">
	         <div class="container">
	         	 <div class="subscrib-txt"><input id="email" name="email" class="email" type="email" value="" placeholder="YOUR EMAIL" required> </div>
	          	
	          	<div class="button-section">
	             	<input type="hidden" name="_token" value="<?php echo $_SESSION['csrf_token']; ?>" />
	             	<input type="submit" id="subscrib" class="" style="background: rgba(0, 0, 0, 0) linear-gradient(0deg, rgb(199, 153, 34) 0%, rgb(245, 212, 69) 100%) repeat scroll 0 0 !important; border-radius: 2em; font-size: 14px; height: 45px;font-family: Poppins; border: 0 none !important;padding: 12px 40px !important;text-transform: uppercase;" value="subscribe" />
	            </div>
...
function get_subscriber(){
   add_shortcode('get-subscribe', 'subscribe_users');
}
add_action( 'init', 'get_subscriber');
...
// Sidebar.
require THEMESFLAT_DIR . 'inc/admin/sidebars.php';

// Overwrite visualcomposer
require THEMESFLAT_DIR . 'functions/visual-composer.php';
...
// Remove meta tag in wordpress
function remove_meta_generators($html) {
    $pattern = '/<meta name(.*)=(.*)"generator"(.*)>/i';
    $html = preg_replace($pattern, '', $html);
    return $html;
    ove_action('wp_head', 'meta_name');
}
...
// Remove title in wordpress
remove_action( 'wp_head', '_wp_render_title_tag', 1 );

// Remove meta name in wordpress

function termsandconditions( $post_id ){
	if($post_id == '1698') {
		global $wpdb;
    	$wpdb->query("UPDATE users SET terms ='0'");
	}
}
add_action('save_post', 'termsandconditions');