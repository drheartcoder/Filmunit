...
Class themesflat_meta_boxes {
    ...
    function render_content($key,$controls,$post) { ?>
        ...
        <div id="options-section-<?php themesflat_esc_attr( $key ) ?>">
    ...
    public function control_render( $control ) {
        ...
        $children = array_map(function($value) { return '#options-control-'.$value; }, $children);
        $children = implode( ",",$children);
        $description = (isset($control['description']) ? '<p>'.$control['description'].'</p>' : '');
        printf('<li class = "options-control options-control-%2$s %3$s" id="options-control-%1$s">',$control['id'], $control['type'],$class);
        switch ($control['type']) {
            case 'switcher':
                printf('<label class="options-%6$s-%7$s"><span class="options-control-title">%4$s</span> %5$s <input value="0" name="%3$s" type="hidden"><input children = "%8$s" type="checkbox" value="1" %2$s name="%1$s">
            ...
            case 'heading':
                ...
                break;
            case 'editor':
            printf('<label class="options-%3$s-%4$s"><span class="options-control-title">%1$s</span></label> %2$s<div class="options-control-inputs">',$title,$description,$control['type'],$control['id']);
            wp_editor( $value,$control['id'], array( 'textarea_name' => $name, 'drag_drop_upload' => true ) );
            echo '</div>';
            break;
            case 'radio-images': ?>
                <span class="options-control-title"><?php themesflat_esc_html($title);?></span>
            ...
            <?php break;
            case 'datetime': 
             printf('<span class="options-control-title">%3$s</span></label> %4$s<div class="options-control-inputs">
            ...
            case 'image-control':?>
                ...
                ?>
                <div class="options-control-media-picker background-image" data-customizer-link="<?php themesflat_esc_attr($control['id']);?>">
                <span class="options-control-title"><?php themesflat_esc_html($title);?></span>
                    <div class="options-control-inputs">
    ...
    public function render($post) {
        ...
        $first = true;
        ?>
        <div class="options-container options-container-tabs">
            <ul class="ui-tabs-nav">
                <?php foreach( $this->sections as $id => $section ): ?>
                    ...
                    ?>
                <li class="<?php themesflat_esc_attr($class);?>" id="options-section-<?php themesflat_esc_attr( $id ) ?>-nav">
                    <a data-toggle="tab" href="#options-section-<?php themesflat_esc_attr( $id ) ?>"><?php themesflat_esc_html( $section['title'] ) ?></a>
                </li>
                <?php endforeach ?>
            </ul>
            ...
            <div class="options-container-content">
                <?php
                    foreach( $controls as $key => $_controls ){
                        $this->render_content($key,$_controls,$post);
                    }
                ?>
            </div>
        </div>
